...
void dec_bin() { //converte de decimal para binario
    ...
    while(check == false) { //checa se os algarismos estao de acordo com os algarismos decimais
        ...
        while(dec_prov[i] != '\0') {
            if(isalpha(dec_prov[i])) {
                ...
                printf("Decimal -> Bin·rio\n");
                printf("-------------------------------------\n");
                printf(">Caracter inv·lido! Insira novamente<\n");
                printf("-------------------------------------\n");
    ...
    if(dec == 0) { 
        ...
        while(dec != 1) { //faz o calculo para descobrir cada bit e armazena no array bin (na ordem lsb -> msb)
            if(dec / 2 == 1) { //se nao tiver mais como dividir, armazena o resto e o resultado da divis√£o
                bin[i] = dec % 2;
                bin[i + 1] = round(dec / 2);
            }
...
void dec_hex() { //converte de decimal para hexadecimal
    ...
    while(check == false) { //checa se os algarismos estao de acordo com os algarismos decimais
        ...
        while(dec_prov[i] != '\0') {
            if(isalpha(dec_prov[i])) {
                ...
                printf("Decimal -> Hexadecimal\n");
                printf("--------------------------------------\n");
                printf(">Caractere inv·lido! Insira novamente<\n");
                printf("--------------------------------------\n");
                break;
            }
            else if(dec_prov[i + 1] == '\0') {
    ...
    if(dec < 16) { //se o numero for menor que 16, imprime seu equivalente hexadecimal diretamente
        ...
        while(dec >= 16) { //faz o calculo para descobrir cada algarismo hexadecimal e o imprime
            if(round(dec / 16) < 16) { //caso nao tenha como dividir mais, ele pega o resto e o resultado da divis√£o
                switch((dec % 16)) {
                    case 10:
                        hex[i] = 'A';
                        break;
...
void bin_dec() { //converte de binario para decimal 
    ...
    i = 0;
    n_casas = 0;
    while(bin[i] != '\0') { //conta o n√∫mero de casas do numero binario
        n_casas++;
        ++i;
    }
...
void bin_oct() { //Converte de binario para octal
    ...
    if( n_casas % 3 == 0 ) { //quando o numero de casas for divisivel por 3
        ...
        quant = n_casas; 
        while(quant % 3 != 0) { //procura o pr√≥ximo numero divisivel por 3 maior que n_casas
            ++quant;
        } 
...
void bin_hex() { //converte de binario para hexadecimal
    ...
    if(n_casas <= 4) { //trata de numeros menores que 15
        ...
        if(n_casas % 4 != 0) { //trata de numeros binarios imcompletos (n√£o divisiveis por 4)
        ...
        for(int i = 0; i < quant; i++) { //divide o numero binario (de 4 em 4) e os converte para decimal
            ...
            if(pos == 0) { //imprime o algarismo hexadecimal e reseta a contagem regressiva que divide o numero binario (de 4 em 4)
                if(reg == false && dec != 0) { //exclue as possiveis divis√µes do binario que s√£o apenas zeros a esquerda
                    reg = true;
                }
                switch(dec) { //imprime o algarismo correspondente ao numero decimal
                    ...
                    case 15:
                        ...
                        break;
                    default:
                        if(reg == true) { //sÛ permite imprimir caso nao seja um zero a esquerda
                            printf("%d", dec);
                            break;
                        }
                        else {
...
void hex_dec() { //converte de hexadecimal para decimal  
    ...
    i = 0;
    n_alg = 0;
    while(hex[i] != '\0') { //conta o n√∫mero de algarismos do num hex
        if(islower(hex[i])) {
            hex[i] = toupper(hex[i]);
        }
        n_alg++;
...
void hex_bin() { //converte de hexadecimal para binario
    ...
    while(check == false) { //pede para o usu√°rio digitar novamente at√© os algarismos estarem de acordo
        printf("Hexadecimal: ");
        scanf("%s", hex);
